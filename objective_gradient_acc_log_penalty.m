...
%     auxdata.mu2_tmp = 1./(1+exp(-auxdata.iter+2));
    running_cost = auxdata.mu1 * 0.1 * sum(A.^2, "all") ...
        + auxdata.mu2 * 0.1 * sum(auxdata.a.*(-atan(auxdata.b.*(h) + auxdata.c) + auxdata.a*pi/2), "all") ...
        + auxdata.mu3 * 0.1 * sum(auxdata.d.*(atan(auxdata.e.*(h) + auxdata.f) + auxdata.d*pi/2), "all"); 
