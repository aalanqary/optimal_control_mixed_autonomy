function [c, dc] = constraint_gradient_min_index(X, V, A, Fx, Fv, Fa, Fu, PQ0, P_dot, Q_dot, auxdata, i)
...
%     drawnow;
    ...
    % Gradient
    if nargout >= 2
        PQ = ode3(@(t,PQ) F_adjoint(P_dot, Q_dot, i, auxdata), flip(auxdata.time), PQ0);
        PQ = flip(PQ,1);
...
%%%%% Adjoint system %%%%%
function PQ_dot = F_adjoint(P_dot, Q_dot, i, auxdata)
...
function dl = phi_partial(t, i, X, V, U, A, var, auxdata)
    ...
    if var == "xh" 
        ...
        if ismembc(i-1, Ih)
            ...
            cond1 = (h< -eps);
            cond2 = and((h<= eps), (h >= - eps));
            phi = -np.ones(length(dh)).*cond1 + (1/(4*eps))*dh.* cond2;
