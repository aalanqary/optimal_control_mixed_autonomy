function [c, dc] = constraint_gradient_min_index(X, V, A, Fx, Fv, Fa, Fu, PQ0, P_dot, Q_dot, auxdata, i, U_vec)
...
%     drawnow;
    ...
    if nargout >= 2
        ...
        % Lu - zeta fu
        dc = Q_short(:, i) + phi_partial(flip(auxdata.time), 0, [], [], U_vec, [],  "u", auxdata)'; 
%         figure(4); 
%         plot(dz)
%         drawnow
    end 
...
%%%%% Adjoint system %%%%%
function PQ_dot = F_adjoint_mod(P_dot, Q_dot, i, auxdata)
