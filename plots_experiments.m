...
% UPDATE PATH
platoon_name = "AV5"
path = "results/real_traj/sequential_platoons_new/" + platoon_name + "/";
load(path+"leader.mat", "leader")
load(path+"auxdata.mat", "auxdata")
load(path+"U.mat")
U_star = U0;

%Path to store results
results_folder = "final_plots/sequential_experiments/" + platoon_name;
if not(isfolder(results_folder))
    mkdir(results_folder)
end
...
plots_list(end+1) = p;
legend_arr(end+1) = "HVs";
title("Sequential Experiment - " + length(auxdata.Ia) + "AVs", "fontsize", 12)
xlabel("Time (s)")
ylabel("Velocity (m/s)")
legend(plots_list, legend_arr)
fig_filename = results_folder + "/velocity";
