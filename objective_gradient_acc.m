...
% 
%     figure(2)
%     [Xlin, ~] = linear_sys_solve(U_vec, auxdata);
...
function dl = L_partial(t, X, V, U, A, var, auxdata)
    ...
    switch var
        case "xh" 
            dl = 2 * A(auxdata.Ih) .* ACC_partial(Xl(auxdata.Ih), X(auxdata.Ih), Vl(auxdata.Ih), V(auxdata.Ih), "x", auxdata) ...
        ...
        case "xa"
            dl = 2 * ismember(auxdata.Ia+1, auxdata.Ih)' .* Af(auxdata.Ia) .* ACC_partial(X(auxdata.Ia), Xf(auxdata.Ia), V(auxdata.Ia), Vf(auxdata.Ia), "xl", auxdata);
        case "vh"
